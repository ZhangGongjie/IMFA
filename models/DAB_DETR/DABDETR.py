...
class DABDETR(nn.Module):
    ...
    def forward(self, samples: NestedTensor):
        ...
        features, pos_embeds, coords = self.backbone(samples)

        feature_pyramid = []
        feature_pyramid_mask = []
        for lvl, feat in enumerate(features):
            f, m = feat.decompose()
            feature_pyramid.append(self.projs[lvl](f))
            feature_pyramid_mask.append(m)

        src, mask = features[-1].decompose()
        assert mask is not None

        # TODO: delete this
        # bs, memory_h, memory_w = src.shape[0], src.shape[2], src.shape[3]
        ...
        hs, reference = self.transformer(self.projs[-1](src),
                                         mask,
                                         embedweight,
                                         pos_embeds[-1],
                                         coords[-1],
