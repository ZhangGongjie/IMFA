...
class Joiner(nn.Sequential):
    ...
    def forward(self, tensor_list: NestedTensor):
        ...
        for name, x in xs.items():
            out.append(x)
            # position encoding & coords
            pos_embed, coord = self[1](x)
            pos_embeds.append(pos_embed.to(x.tensors.dtype))
            coords.append(coord.to(x.tensors.dtype))

        return out, pos_embeds, coords
...
def build_backbone(args):
    ...
    backbone = Backbone(args.backbone, train_backbone, return_interm_layers, args.dilation, batch_norm=FrozenBatchNorm2d)
